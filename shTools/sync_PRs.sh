...
sync_pulls() {
  ...
  local github_issues_url="https://api.github.com/repos/${github_path}/pulls?state=all&per_page=100"
  local forgejo_issues_url="${forgejo_base}/api/v1/repos/${forgejo_path}/pulls"

  curl -sS \
    -H "Authorization: Bearer ${PRODUCER_DESTINATION_TOKEN}" \
    -H "Accept: application/vnd.github+json" \
    "$github_issues_url" \
  | python3 ./pyTools/json_lines.py title body state \
  ...
  do
    ...
    issue_state="$(printf '%s' "$issue_json" | python3 ./pyTools/son_finder.py state)"
    issue_number="$(printf '%s' "$create_response" | python3 ./pyTools/json_finder.py number)"

    echo "created pull #$issue_number with state=$issue_state"
